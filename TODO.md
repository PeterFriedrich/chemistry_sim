...
- [ ] **Owner walkthrough of every sim against hand calculations.** SPEC_phase1.md §5 criterion 5: default settings for each sim, compute the readouts by hand with the booklet, confirm they match. Done when each sim has a ✓ (or a bug filed) here. `calorimetry`: ☐
...
- [ ] Chemistry 30 A: `hess` — needs the Δ<sub>f</sub>H° table transcribed.
...
- [ ] Replace the physics colour tokens inherited in `site/css/style.css` / `lib/canvas.js` `theme()` (`--c-velocity` … `--c-total`) with a chemistry colour code once the second sim needs one (ARCHITECTURE.md §5).
